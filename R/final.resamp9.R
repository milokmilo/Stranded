#' Final resampling
#'
#' Final resampling. Modified from final.resamp.R of HPbayes_0.1 package
#' @param K
#' @param B1
#' @param H.new
#' @param H.k
...
final.resamp9 <- function (K, B1, H.new, H.k, log.like, d.keep, prior, h.mu, h.sig, 
                           nrisk, ndeath, B = 400, theta.dim = 9, age = c(1e-05, 1, 
                                                                          seq(5, 100, 5))) {
  lx <- nrisk
  dx <- ndeath
  ...
  for (k in 1:K) {
    ...
    DENS.MAT[, 1] <- (B0/n.k) * dens.prior9(H.k, pri.lo = prilo, pri.hi = prihi)
    ...
    like.k <- exp(log.like)
    wts.k.0 <- like.k * dens.prior9(H.k, pri.lo = prilo, pri.hi = prihi)
    wts.k <- wts.k.0/qk
