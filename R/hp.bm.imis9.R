#' Heligman Pollard imis
#'
#' Heligman Pollard imis. Modified function from hp.bm.imis.R of HPbayes_0.1
#' @param prior
#' @param nrisk
#' @param ndeath
#' @param K
#' @param d. Default = 10
#' @param B. Default = 400
#' @param age. Default c(1e-05, 1, seq(5, 100, 5)), CI = 95)
#' @keywords leslie matrix generation
#' @export
#' @examples
#' hp.bm.imis9()

hp.bm.imis9 <- function (prior, nrisk, ndeath, K, d = 10, B = 400, age = c(1e-05, 
                                                                           1, seq(5, 100, 5)), CI = 95) {
  low <- apply(prior, 2, min)
  high <- apply(prior, 2, min)
  opt.result <- loop.optim9(prior = prior, nrisk = nrisk, ndeath = ndeath, 
  ...
  samp.po <- samp.postopt9(opt.cov.d = opt.cov.d, opt.mu.d = opt.mu.d, 
                           prior = prior, d.keep = d.keep, B = B, d = d, B0 = nrow(prior))
  H.k <- samp.po$H.k
  H.new <- samp.po$H.new
  B1 <- samp.po$B1
  ll.postopt <- like.resamp9(K = K, log.like.0 = log.like.0, 
  ...
  K <- ll.postopt$K
  result <- final.resamp9(K = K, B1 = B1, B = B, H.new = H.new, 
